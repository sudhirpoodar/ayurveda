...
(function($) {
  ...
  $.redux.active_demo_import = function() {
    let searchParams = new URLSearchParams(window.location.search);
    var demoTarget = $('#20_section_group_li');
    var blockTarget = $('#20_section_group');
    if (searchParams.has('demo_import')) {
      $('.redux-group-menu li').removeClass('active');
      $('.redux-group-tab').css({ 'display': 'none' });
      demoTarget.addClass('active').trigger('click');
      blockTarget.css({ 'display': 'block' });
    } else {
      demoTarget.removeClass('active');
