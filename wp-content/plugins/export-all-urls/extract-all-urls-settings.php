...
?>
                        ...
                        <tr class="advance-options" style="display: none">
                            ...
                            <td>
                                ...
                                <label><span style="cursor: not-allowed; -moz-user-select: none; -webkit-user-select: none; -ms-user-select:none; user-select:none;-o-user-select:none;" unselectable="on" onselectstart="return false;" onmousedown="return false;"><?php echo get_home_path(); ?>wp-content/uploads</span><input type="text" name="csv-file-name" placeholder="An Error Occured" value="<?php echo $file_path['subdir']."/".$file_name; ?>" size="30%" /></label><br/>
...
<?php
    ...
    if (isset($_POST['export'])) {
        ...
        if (!empty($_POST['post-type']) && !empty($_POST['export-type']) && !empty($_POST['additional-data']) && !empty($_POST['post-status']) && !empty($_POST['post-author']) && !empty($_POST['number-of-posts']) ) {
            ...
            generate_output($selected_post_type, $post_status, $post_author, $post_per_page, $offset, $export_type, $additional_data, $csv_path, $csv_name);
